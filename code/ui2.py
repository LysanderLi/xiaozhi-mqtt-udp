...
dian_dsc.init()
dian_dsc.color = lv.color_hex(0xf4ea2a)
...
circle_clear_dsc.color = lv.color_hex(0xf4ea2a)
...
clear_dsc.bg_color = lv.color_white()  # 使用背景色（白色）来清除内容

# Create circle style
cloud_dsc = lv.draw_arc_dsc_t()
cloud_dsc.init()
cloud_dsc.color = lv.color_hex(0xf4ea2a)
cloud_dsc.width = 3  # 圆的粗细，设置较大值可填充圆形
cloud_dsc.opa = lv.OPA.COVER

question_dsc = lv.draw_arc_dsc_t()
question_dsc.init()
question_dsc.color = lv.color_hex(0xf4ea2a)
...
speaking_screen.set_style_bg_color(lv.color_hex(0xffffff), lv.PART.MAIN|lv.STATE.DEFAULT)
...
speaking_canvas.fill_bg(lv.color_white(), lv.OPA.COVER)
...
angry_canvas.center()
angry_canvas.fill_bg(lv.color_white(), lv.OPA.COVER)
...
# 绘制云朵
cloud_x = 210  # 云朵中心X坐标（右上角）
cloud_y = 30   # 云朵中心Y坐标（右上角）
cloud_radius = 15  # 云朵的半径（缩小一半）
...
def angry_anim_timer_cb(timer):
    ...
    # 绘制云朵（在眼睛上方，需要重新绘制）
    # 顶部弧形
    angry_canvas.draw_arc(cloud_x - 15, cloud_y - 5, cloud_radius - 5, 0, 360, cloud_dsc)  
    angry_canvas.draw_arc(cloud_x - 5, cloud_y - 8, cloud_radius - 3, 0, 360, cloud_dsc)  
    angry_canvas.draw_arc(cloud_x + 5, cloud_y - 8, cloud_radius - 3, 0, 360, cloud_dsc)   
    angry_canvas.draw_arc(cloud_x + 15, cloud_y - 5, cloud_radius - 5, 0, 360, cloud_dsc)  

    # 中间主弧形
    angry_canvas.draw_arc(cloud_x - 10, cloud_y, cloud_radius, 0, 360, cloud_dsc)  # 左侧弧形
    angry_canvas.draw_arc(cloud_x, cloud_y, cloud_radius, 0, 360, cloud_dsc)        # 中间弧形
    angry_canvas.draw_arc(cloud_x + 10, cloud_y, cloud_radius, 0, 360, cloud_dsc)   # 右侧弧形
...
listening_canvas.fill_bg(lv.color_white(), lv.OPA.COVER)
...
question_img.set_src("U:/media/question_mark_black.png")
...
sleeping_canvas.fill_bg(lv.color_white(), lv.OPA.COVER)
...
z_line_dsc.init()
z_line_dsc.color = lv.color_hex(0xf4ea2a)
z_line_dsc.width = 3
...
open_eye_canvas.fill_bg(lv.color_white(), lv.OPA.COVER)
